...
Below is the syntax highlighted version of PokerHand.java.
...
public class PokerHand {
    ...
    public boolean fullHouse() {
        ...
        for (int i = 1; i < HANDSIZE - 1; i++) {
            ...
            if (hand[i].charAt(0) == rank2) sumRank2++;
        }

        // is it a full house?
        if (sumRank1 == 3 && sumRank2 == 2) return true;
        if (sumRank1 == 2 && sumRank2 == 3) return true;
        // no full house :(
        return false;
    }
...
-- PokerHand data type
data PokerHand = PokerHand { hand :: [String] } deriving (Show)

-- constructor reads a 5-card poker hand from StdIn
readPokerHand :: IO PokerHand
readPokerHand = do
  hand <- sequence [getLine | _ <- [1..5]]
  return (PokerHand (sort hand))

-- is this hand a flush?
flush :: PokerHand -> Bool
flush (PokerHand hand) = all (\x -> last x == last (head hand)) hand

-- is this hand a full house?
fullHouse :: PokerHand -> Bool
fullHouse (PokerHand hand) = (length (filter (== head hand) hand) == 3) && (length (filter (== last hand) hand) == 2)

-- string representation of this hand sorted
instance Show PokerHand where
  show (PokerHand hand) = unwords hand
...
main = do
  ...
  let hands = lines input
  let pokerHands = map (\x -> readPokerHand x) hands
  let flushes = filter flush pokerHands
  let fullHouses = filter fullHouse pokerHands
  mapM_ (\x -> putStrLn ("     Flush: " ++ show x)) flushes
  mapM_ (\x -> putStrLn ("Full house: " ++ show x)) fullHouses